...
class MonitoringStrategy(IStrategy):
    ...
    Monitors 6 portfolio coins with various technical indicatoasdfasdfrs
    ...
    def _check_and_send_signals(self, dataframe: DataFrame, pair: str):
        ...
        if previous['ma_20'] <= previous['ma_50'] and current['ma_20'] > current['ma_50']:
            signals.append({
                ...
                'value': {'ma_20': current['ma_20'], 'ma_50': current['ma_50']},
                'strength': 'high'
            })
        ...
        if previous['macd'] <= previous['macdsignal'] and current['macd'] > current['macdsignal']:
            signals.append({
                ...
                'value': {'macd': current['macd'], 'signal': current['macdsignal']},
                'strength': 'medium'
            })
        ...
        if current['volume'] > current['volume_sma'] * 2:
            signals.append({
                'type': 'volume_spike',
                'message': f'{pair} Volume spike detected: {current["volume"]:.0f} vs avg {current["volume_sma"]:.0f}',
                'value': {'volume': current['volume'], 'avg_volume': current['volume_sma']},
                'strength': 'medium'
            })
        ...
        if current['close'] > current['bb_upper']:
            signals.append({
                ...
                'value': {'price': current['close'], 'bb_upper': current['bb_upper']},
                'strength': 'medium'
            })
            ...
            signals.append({
                ...
                'value': {'price': current['close'], 'bb_lower': current['bb_lower']},
                'strength': 'medium'
            })
        ...
        if current['stoch_k'] < 20 and current['stoch_d'] < 20:
            signals.append({
                ...
                'value': {'stoch_k': current['stoch_k'], 'stoch_d': current['stoch_d']},
                'strength': 'medium'
            })
